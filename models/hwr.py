...
class CRNN(nn.Module):
    ...
    def forward(self, input, online=True):
        conv = self.cnn(input)
        b, c, h, w = conv.size()
        ...
        if online:
            online = torch.ones(conv.shape[0], conv.shape[1], 1)
        else:
            online = torch.zeros(conv.shape[0], conv.shape[1], 1)

        rnn_input = torch.cat((conv, online), dim=2)
